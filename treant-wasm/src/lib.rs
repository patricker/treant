use wasm_bindgen::prelude::*;

mod amazons;
mod baghchal;
mod bullscows;
mod capturego;
mod chomp;
mod climb;
mod clobber;
mod col;
mod connectfour;
mod counting;
mod dice;
mod difficulty;
mod domineering;
mod dotsboxes;
mod draughts;
mod euclid;
mod foxhounds;
mod frontline;
mod gale;
mod game2048;
mod gridlib;
mod gridpack;
mod hex;
mod konane;
mod lenchoa;
pub mod mancala;
mod mutorere;
mod nim;
mod ninemorris;
mod nogo;
mod oware;
mod pawnduel;
mod pig;
mod pinchfive;
mod prior;
mod quadline;
mod reversi;
mod salvo;
mod shift;
mod sim;
mod slimetrail;
mod strand;
mod subtractsquare;
mod surakarta;
mod tictactoe;
mod toadsfrogs;
mod trails;
mod treblecross;
mod types;
mod vanguard;
mod worldthrees;
mod wythoff;
mod y;

pub use amazons::AmazonsWasm;
pub use baghchal::BaghchalWasm;
pub use bullscows::BullsCowsWasm;
pub use capturego::CaptureGoWasm;
pub use chomp::ChompWasm;
pub use climb::ClimbWasm;
pub use clobber::ClobberWasm;
pub use col::ColWasm;
pub use connectfour::ConnectFourWasm;
pub use counting::CountingGameWasm;
pub use dice::DiceGameWasm;
pub use domineering::DomineeringWasm;
pub use dotsboxes::DotsBoxesWasm;
pub use draughts::DraughtsWasm;
pub use euclid::EuclidWasm;
pub use foxhounds::FoxHoundsWasm;
pub use frontline::FrontlineWasm;
pub use gale::GaleWasm;
pub use game2048::Game2048Wasm;
pub use gridpack::{Connect6Wasm, NotaktoWasm, OrderChaosWasm, SquareUpWasm, SquavaWasm};
pub use hex::HexWasm;
pub use konane::KonaneWasm;
pub use lenchoa::LenChoaWasm;
pub use mancala::MancalaWasm;
pub use mutorere::MuTorereWasm;
pub use nim::NimWasm;
pub use ninemorris::NineMorrisWasm;
pub use nogo::NoGoWasm;
pub use oware::OwareWasm;
pub use pawnduel::PawnDuelWasm;
pub use pig::PigWasm;
pub use pinchfive::PinchFiveWasm;
pub use prior::{PriorGamePuctWasm, PriorGameUctWasm};
pub use quadline::QuadlineWasm;
pub use reversi::ReversiWasm;
pub use salvo::SalvoWasm;
pub use shift::ShiftWasm;
pub use sim::SimWasm;
pub use slimetrail::SlimetrailWasm;
pub use strand::StrandWasm;
pub use subtractsquare::SubtractSquareWasm;
pub use surakarta::SurakartaWasm;
pub use tictactoe::TicTacToeWasm;
pub use toadsfrogs::ToadsFrogsWasm;
pub use trails::TrailsWasm;
pub use treblecross::TreblecrossWasm;
pub use vanguard::VanguardWasm;
pub use worldthrees::WorldThreesWasm;
pub use wythoff::WythoffWasm;
pub use y::YGameWasm;

#[wasm_bindgen(start)]
pub fn init() {
    console_error_panic_hook::set_once();
}

#[wasm_bindgen]
pub fn ping() -> String {
    "treant-wasm ready".into()
}
