...
**Gumbel search.**
A root-level search algorithm that uses Gumbel noise sampling and Sequential Halving to select actions. Produces a monotonically improving policy -- more simulations always improve the output. Implemented in the `treant-gumbel` crate. Single-threaded. Below the root, standard PUCT selection is used. Based on Danihelka et al., "Policy improvement by planning with Gumbel" (ICLR 2022). See [Gumbel Search tutorial](../tutorials/08-gumbel-search.md) and [Tree Policies](../concepts/tree-policies.md).
