...
export function makeMoveBoard(pieceKind: 'pawn' | 'disc') {
  return function MoveBoardInner({ board, params, interactive, legalMoves, winCells, lastCells, onMove }: BoardProps) {
    const { t } = useT();
        ...
        <div className={styles.tttGrid} style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
          {cells.map((ch, i) => {
            ...
            return (
              <button
                key={i}
                className={`${styles.tttCell} ${isSel ? styles.shiftSel : ''} ${isTarget ? styles.moveTarget : ''} ${i === shake ? styles.shakeCell : ''} ${wins.has(i) ? styles.winCell : ''} ${last.has(i) ? styles.lastCell : ''}`}
