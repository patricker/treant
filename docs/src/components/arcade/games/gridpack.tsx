...
export function cellHandle(g: any, cols: number, rows: number): GameHandle {
  return {
    ...
    // Only line-win engines expose winning_cells; others skip the glow.
    winningCells: g.winning_cells ? () => splitCells(g.winning_cells()) : undefined,
    free: () => g.free(),
  };
}

/** Parse the engine's comma-joined winning-cell string into indices ("" → []). */
...
export const trapThree: GameDefinition = {
  ...
  knobs: [
    { key: 'cols', label: 'Width', min: 4, max: 12, step: 1 },
    { key: 'rows', label: 'Height', min: 4, max: 12, step: 1 },
  ],
...
export const orderChaos: GameDefinition = {
  ...
  knobs: [
    { key: 'cols', label: 'Width', min: 5, max: 12, step: 1 },
    { key: 'rows', label: 'Height', min: 5, max: 12, step: 1 },
  ],
