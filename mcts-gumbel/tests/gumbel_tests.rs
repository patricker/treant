...
fn nim_finds_optimal_move_from_4() {
    let state = NimGame { stones: 4, current: 0 };
    let config = GumbelConfig { m_actions: 4, seed: 42, ..Default::default() };
    ...
    assert!(result.root_value > 0.0, "winning position should have positive value");
}

/// From N=3, both moves lose. Search should still return a valid move.
#[test]
fn nim_returns_move_from_losing_position() {
    let state = NimGame { stones: 3, current: 0 };
    let config = GumbelConfig { m_actions: 4, seed: 42, ..Default::default() };
    let mut search = GumbelSearch::new(NimEvaluator, config);
...
fn nim_finds_optimal_move_from_7() {
    let state = NimGame { stones: 7, current: 0 };
    let config = GumbelConfig { m_actions: 4, seed: 99, ..Default::default() };
    let mut search = GumbelSearch::new(NimEvaluator, config);
...
#[test]
fn nim_deeper_game() {
    let state = NimGame { stones: 10, current: 0 };
    let config = GumbelConfig { m_actions: 4, seed: 42, ..Default::default() };
    let mut search = GumbelSearch::new(NimEvaluator, config);
...
fn wide_game_sequential_halving() {
    let state = WideGame { width: 8, depth: 4, current: 0 };
    let config = GumbelConfig { m_actions: 8, seed: 42, ..Default::default() };
...
fn wide_game_m_larger_than_moves() {
    let state = WideGame { width: 4, depth: 3, current: 0 };
    let config = GumbelConfig { m_actions: 16, seed: 42, ..Default::default() };
    let mut search = GumbelSearch::new(WideEvaluator, config);
    let result = search.search(&state, 100);
...
fn single_move_returns_immediately_with_value() {
    let state = NimGame { stones: 1, current: 0 };
...
fn zero_simulations() {
    let state = NimGame { stones: 4, current: 0 };
    let config = GumbelConfig { m_actions: 2, seed: 42, ..Default::default() };
    let mut search = GumbelSearch::new(NimEvaluator, config);
...
fn budget_not_exceeded() {
    ...
    for budget in [1, 2, 3, 5, 10, 50] {
        ...
        assert!(
            ...
            "budget={budget}, used={}", result.simulations_used
        );
    }
}
...
fn panics_on_terminal_state() {
    let state = NimGame { stones: 0, current: 0 };
    let mut search = GumbelSearch::new(NimEvaluator, GumbelConfig::default());
...
fn m_actions_one() {
    let state = NimGame { stones: 4, current: 0 };
    let config = GumbelConfig { m_actions: 1, seed: 42, ..Default::default() };
    let mut search = GumbelSearch::new(NimEvaluator, config);
...
fn improved_policy_is_valid_distribution() {
    let state = NimGame { stones: 5, current: 0 };
    let config = GumbelConfig { m_actions: 4, seed: 42, ..Default::default() };
    let mut search = GumbelSearch::new(NimEvaluator, config);
    let result = search.search(&state, 200);

    let sum: f64 = result.move_stats.iter().map(|s| s.improved_policy).sum();
    assert!((sum - 1.0).abs() < 1e-10, "improved policy should sum to 1.0, got {sum}");
    for s in &result.move_stats {
        assert!(s.improved_policy >= 0.0, "policy probability should be non-negative");
    }
}
...
fn completed_q_values_populated() {
    ...
    let q_take1 = result.move_stats.iter().find(|s| s.mov == NimMove(1)).unwrap().completed_q;
    let q_take2 = result.move_stats.iter().find(|s| s.mov == NimMove(2)).unwrap().completed_q;
    assert!(q_take1 > q_take2, "Take(1) Q={q_take1} should be > Take(2) Q={q_take2}");
}
...
fn deterministic_with_same_seed() {
    let state = NimGame { stones: 5, current: 0 };
    let config = GumbelConfig { seed: 777, m_actions: 4, ..Default::default() };
...
fn set_seed_reproducible() {
    let state = NimGame { stones: 5, current: 0 };
    let config = GumbelConfig { seed: 42, m_actions: 2, value_scale: 1.0, ..Default::default() };
    let mut search = GumbelSearch::new(NimEvaluator, config);
...
fn different_seeds_produce_different_results() {
    ...
    for seed in 0..50 {
        let config = GumbelConfig { seed, m_actions: 2, value_scale: 1.0, ..Default::default() };
        let mut search = GumbelSearch::new(NimEvaluator, config);
    ...
    impl GumbelEvaluator<NimGame> for MisleadingEval {
        fn evaluate(&self, state: &NimGame, moves: &[NimMove]) -> (Vec<f64>, f64) {
            ...
            let logits: Vec<f64> = moves.iter().map(|m| {
                if m.0 == 2 { 5.0 } else { -5.0 }
            }).collect();
            let value = if state.stones.is_multiple_of(3) { -0.5 } else { 0.5 };
            (logits, value)
        }
    }
